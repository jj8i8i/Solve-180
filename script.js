...
function displayResults(result) {
    // กรองผลลัพธ์ที่ไม่ซ้ำกันโดยเช็คจากสมการ (solution.str)
    const uniqueSolutions = Array.from(
        new Map(result.solutions.map(sol => [sol.str, sol])).values()
    );
...
function generateDerivationList(item) {
    ...
    steps.forEach(stepStr => {
        ...
        try {
            ...
            katex.render(finalStr, li, {
                throwOnError: false,
                displayMode: true
            });
        } catch (e) {
            li.textContent = `${stepStr} = ${item.value}`;
        }
        list.appendChild(li);
    });
    ...
    try {
        ...
        katex.render(finalStr, finalLi, {
            throwOnError: false,
            displayMode: true
        });
    } catch (e) {
        finalLi.textContent = `${finalEquationStr} = ${item.value}`;
    }
